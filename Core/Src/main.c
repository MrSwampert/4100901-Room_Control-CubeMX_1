...
#define PASSWORD "D85#" // Contraseña de 4 dígitos
...
void process_key(uint8_t key)
{
    ...
    if (password_index == PASSWORD_LEN) {
        if (strncmp(entered_password, PASSWORD, PASSWORD_LEN) == 0) {
            ...
            led_on(&led_ext);
            led_timer_start = HAL_GetTick();
            led_on_duration = 3000; // 3 segundos para coincidir con el mensaje de bienvenida
            
            // Esperar 3 segundos antes de mostrar el mensaje de bienvenida
            HAL_Delay(3000);
            printf("¡Bienvenido Santiago!\r\n");
        } else {
